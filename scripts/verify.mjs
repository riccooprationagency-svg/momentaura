 ...
 *
 *   V1  dependency surface — Astro at runtime, one named devDependency
 ...
 *
 * This check asserted "astro, and no devDependencies at all" until step 12. That
 * was an absence, and an absence needs no judgement to check. It is now a named
 * list, which is a budget, and the same thing that happened to V4 at step 7 has
 * now happened here: the loosening has to buy something, and it has to be argued
 * in the file rather than remembered.
 *
 * STEP 12 ADDED ONE NAME: sharp, in devDependencies, for scripts/images.mjs.
 *
 * WHY IT IS A NAME AND NOT A COUNT. `dev.length <= 1` accepts any one package.
 * A list accepts sharp. Adding a second dependency has to be done here, on
 * purpose, in a commit that says so — which is the whole reason a list is worth
 * more than a number, and the same argument V3 makes about pinning the accent to
 * named selectors rather than to a count of two.
 *
 * WHY IT IS DECLARED AT ALL, given it was already reachable. Astro carries sharp
 * in its own optionalDependencies, so `import "sharp"` worked in this tree before
 * anything was added to package.json. Borrowing it would have kept V1 an absence
 * at the cost of pinning our image pipeline to a version another project picked,
 * from a dependency marked optional — a tree where the install skipped it is a
 * valid tree, and nothing would have said so until an encode produced different
 * bytes. A tool that decides what a photograph looks like names its encoder.
 *
 * WHAT THE WIDENING BOUGHT, and it is the part that limits it: AVIF and WebP at
 * three widths from one 3:4 crop, generated by a script rather than by a person
 * remembering six files per photograph. The alternative on the table was hand-
 * processing outside the repo, which keeps V1 an absence and turns "one crop,
 * one light, one angle, across every product" from something code enforces into
 * something someone recalls.
 *
 * WHAT IT DID NOT BUY, and this is why the cost is as low as it is: nothing that
 * ships. sharp is a devDependency, `npm run build` never imports it, the
 * renditions are committed to public/img/, and a deploy from a tree installed
 * with --omit=dev produces byte-identical output. If sharp breaks, it breaks at
 * a keyboard.
 *
 * BEFORE ADDING A SECOND NAME: the answer is almost always no, and it is no by
 * default for anything the site loads at runtime — that half of the rule did not
 * move and is not negotiable. Argue it here, beside this one, or do not add it.
 */

{
  ...
  // Runtime: astro and nothing else. Unmoved.
  const extra = deps.filter((d) => d !== "astro");
  if (extra.length) {
    fail("V1", `unexpected dependencies: ${extra.join(", ")}`);
    fail("V1", "  Nothing the site loads at runtime has ever been permitted here.");
  }

  // Build-time: exactly the names below, each argued in the comment above.
  const DEV_ALLOWED = new Set(["sharp"]);
  const unnamed = dev.filter((d) => !DEV_ALLOWED.has(d));
  if (unnamed.length) {
    fail("V1", `undeclared devDependencies: ${unnamed.join(", ")}`);
    fail("V1", `  Permitted: ${[...DEV_ALLOWED].join(", ")}. A new one is a decision, not an install —`);
    fail("V1", "  add it to DEV_ALLOWED with its argument in the comment above, or drop it.");
  }
  ...
  const missing = [...DEV_ALLOWED].filter((d) => !dev.includes(d));
  if (missing.length) {
