 ...
 *
 *   V1  dependency surface — Astro at runtime, one named pinned build tool
 ...
 *   V14 images: reserved space, resolvable URLs, and a 200KB ceiling
 *
 ...
 *
 * ---------- RUNTIME: ASTRO, AND NOTHING ELSE ----------
 *
 * Unchanged, and this is the half that matters. Nothing the browser loads has
 * ever been permitted here and nothing is going to be. It is what keeps the
 * shipped bundle at zero JavaScript beyond the cart: V4 counts the one script
 * and holds it to a budget, but V4 only ever gets to count one file because
 * this list has one name on it. A runtime dependency is how a site acquires a
 * second bundle, and it is not on the table.
 *
 * ---------- BUILD TIME: SHARP ONLY, PINNED ----------
 *
 * One name, at an exact version rather than a range, used by scripts/images.mjs
 * and by nothing else. It never reaches a browser: it runs at a keyboard to
 * produce AVIF and WebP at three widths from one 3:4 crop, and the renditions it
 * writes are committed to public/img/. `npm run build` never imports it, and a
 * deploy from a tree installed with --omit=dev emits byte-identical output.
 *
 * Pinned because it decides what a photograph looks like. A caret range lets a
 * future install change the encoder, and the same source frame through a
 * different libvips is different bytes at a different size — a silent change to
 * the thing this repo is most careful about. The check below asserts the pin, so
 * "pinned" is a fact rather than a sentence in a comment.
 *
 * ---------- WHY DECLARED, WHEN IT WAS ALREADY REACHABLE ----------
 *
 * Astro carries sharp in its own optionalDependencies, so `import "sharp"` worked
 * in this tree before anything was added to package.json. Borrowing it would have
 * kept this check reading "astro and nothing else" while the image pipeline
 * depended on a package nothing here declares, that Astro chose the version of,
 * that Astro could drop in a minor release, and that is marked optional — so a
 * tree where the install skipped it is a valid tree and nothing would have said
 * so until an encode produced different bytes.
 *
 * That is a gate reporting a state it cannot see, which is the one failure this
 * repo keeps finding and fixing. V3 counted accent references until a count of
 * two would have accepted any two. V9 stopped maintaining PAIRINGS by memory
 * after four tokens shipped unmeasured behind a comment claiming otherwise. V11
 * exists because CLAUDE.md printed two hex values the source contradicted. Three
 * corrections, one shape: the report was confident about something it had no way
 * to check. A borrowed sharp would have been the fourth, and it is not being
 * introduced on purpose.
 *
 * ---------- THE COST, STATED ----------
 *
 * This check used to assert an absence — "no devDependencies at all" — which
 * needs no judgement. It now holds a list, which is a budget, and a budget nobody
 * counts becomes a comment. Same move V4 made at step 7 when the cart arrived,
 * and it carries the same obligation: the loosening has to buy something, and the
 * argument has to live in the file rather than in someone's memory.
 *
 * WHAT IT BOUGHT: AVIF and WebP at three widths from one centred 3:4 crop,
 * generated by a script that refuses to upscale, refuses to invent alt text and
 * refuses to write a file over 200KB — rather than by a person remembering six
 * files per photograph. The alternative on the table was processing images by
 * hand outside the repo, which keeps this check an absence and turns "one crop,
 * one light, one angle, across every product" from something code enforces into
 * something someone recalls.
 *
 * WHAT IT COST: the sentence above stopped being true as written. "astro and
 * nothing else" is now "astro at runtime, one named tool at build time", and
 * every future reader has to hold two rules instead of one.
 *
 * ---------- ADDING A SECOND NAME ----------
 *
 * The next name argues its case HERE, in this comment, with what it buys and what
 * it costs, in the commit that adds it. An allow-list of one is still a gate. One
 * that grows without an argument each time is a comment.
 *
 * It is a list and not `dev.length <= 1` for that reason: a count accepts any one
 * package and would let a swap happen silently. A name has to be typed here on
 * purpose, which is the same argument V3 makes about pinning the accent to named
 * selectors rather than to a count of two.
 */

{
  ...
  // Runtime: astro and nothing else. Unmoved, and the half that matters.
  const extra = deps.filter((d) => d !== "astro");
  if (extra.length) {
    fail("V1", `unexpected runtime dependencies: ${extra.join(", ")}`);
    fail("V1", "  Nothing the browser loads has ever been permitted here. This is what keeps");
    fail("V1", "  the shipped output at one script — V4 can only count one file because this");
    fail("V1", "  list has one name on it.");
  }

  // Build time: exactly the names below, each argued in the comment above, each
  // at an exact version.
  const DEV_ALLOWED = new Map([["sharp", "0.35.4"]]);

  const unnamed = dev.filter((d) => !DEV_ALLOWED.has(d));
  if (unnamed.length) {
    fail("V1", `undeclared devDependencies: ${unnamed.join(", ")}`);
    fail("V1", `  Permitted: ${[...DEV_ALLOWED.keys()].join(", ")}. A new one is a decision, not an install.`);
    fail("V1", "  Add it to DEV_ALLOWED with its argument in the comment above — what it buys");
    fail("V1", "  and what it costs — or drop it. An allow-list that grows without an argument");
    fail("V1", "  each time is a comment.");
  }

  // The list is not a wish. A name nobody installed is a rule about nothing, and
  // it would sit here inviting the next package to join it.
  const missing = [...DEV_ALLOWED.keys()].filter((d) => !dev.includes(d));
  if (missing.length) {
    fail("V1", `DEV_ALLOWED names ${missing.join(", ")}, which package.json does not carry`);
    fail("V1", "  Remove the name and its argument. A permission nothing uses only ever grows.");
  }

  // Pinned, not ranged. A caret on an image encoder lets a future install change
  // what a photograph looks like without anything here saying so.
  for (const [name, pin] of DEV_ALLOWED) {
    const declared = pkg.devDependencies?.[name];
    if (declared === undefined) continue;
    if (declared !== pin) {
      fail("V1", `devDependency ${name} is declared "${declared}", expected the exact pin "${pin}"`);
      fail("V1", "  A range lets an install change the encoder, and the same source frame through");
      fail("V1", "  a different libvips is different bytes at a different size. Pin it, or change");
      fail("V1", "  the pin here on purpose.");
    }
  }

  const shown = dev.map((d) => `${d}@${pkg.devDependencies[d]}`).join(", ");
  console.log(`  V1  dependencies      runtime: ${deps.join(", ") || "(none)"}${dev.length ? `  |  build: ${shown}` : ""}`);
}
 ...
 *
 * A media-condition list is exempt wherever it is written, for that identical
 * reason and only for it. Step 12 added the second place the language has one:
 * the `sizes` attribute on <source> and <img> in Gallery.astro, which is a
 * prelude's grammar in an HTML attribute and takes a var() exactly as well as a
 * prelude does — not at all. Without it srcset does nothing, because a browser
 * with no width to work from assumes 100vw and fetches the 1200px file on every
 * phone, which is the opposite of what the widths exist for.
 *
 * The carve-out is the condition, not the tag and not the file: a quoted string
 * containing (min-width: or (max-width: is a media query list, and the lengths
 * paired with those conditions are part of it. Everything else on the line is
 * still matched, so a raw dimension sitting beside one is still caught. The
 * breakpoints inside are the same three declared at the top of tokens.css, and
 * they are read and changed there, exactly as the preludes are.
 *
  ...
  for (const file of sourceFiles.filter((f) => !f.endsWith("dev-guards.css"))) {
    stripAllComments(readFileSync(file, "utf8"))
      ...
      .forEach((rawLine, i) => {
        ...
        // Drop the media prelude and any media-condition list before matching,
        // keeping everything else on the line so a declaration or an attribute
        // beside them is still checked.
        const line = rawLine
          .replace(/@media[^{]*\{?/g, "")
          .replace(/(["'`])(?:(?!\1)[^\\])*\((?:min|max)-width:(?:(?!\1)[^\\])*\1/g, "");
...
if (existsSync(DIST)) {
  ...
  /* ---------- what each page's photographs cost it ----------
   *
   * Until step 12 this budget counted markup, fonts and the cart, and no image
   * bytes at all — because there were no images. It would have kept passing the
   * day photography landed: five reveals at 70KB each is 350KB of a 500KB
   * homepage budget arriving without a single number on this report moving. A
   * budget blind to the largest thing it is meant to weigh is a comment.
   *
   * ONE FILE PER <img>, NOT ALL SIX. A <picture> offers six renditions and the
   * browser fetches exactly one, so charging the page for all six would be a
   * number no visitor ever pays, and a budget nobody can hit is ignored as fast
   * as one nobody counts.
   *
   * WHICH ONE: the largest WebP candidate. That is the worst realistic case and
   * the target case at the same time, which is the only reason it is worth
   * using. Worst, because AVIF is the smaller of the two formats and WebP is
   * what a browser falls back to. Target, because CLAUDE.md aims this budget at
   * a phone on a metered Kenyan bundle, and a phone at 400px CSS width on a 3x
   * screen asks for 1200px — the largest candidate is what it gets, not the
   * smallest. Counting the 400w file would flatter the number on the exact
   * device it exists to protect.
   */
  const imageCost = (page) => {
    const source = readFileSync(page, "utf8");
    const sizeOf = (url) => {
      const f = join(DIST, url.replace(/^\//, ""));
      return url.startsWith("/") && existsSync(f) ? bytes(f) : 0;
    };
    /* Candidates offered by one element: every srcset entry plus the img src. */
    const heaviest = (block) => {
      const urls = [];
      ...
      for (const [, src] of block.matchAll(/<img\b[^>]*\ssrc=["']([^"']+)["']/gi)) urls.push(src);
      /* WebP is the fallback format, so it is the heavier of the two on offer
         and the one a browser without AVIF is served. If an element offers no
         WebP at all, weigh whatever it does offer. */
      const webp = urls.filter((u) => u.endsWith(".webp"));
      return Math.max(0, ...(webp.length ? webp : urls).map(sizeOf));
    };

    let total = 0;
    /* <picture> first, as one element with one fetch, then whatever bare <img>
       is left once those blocks are removed — so an image inside a picture is
       never counted twice. */
    const pictures = [...source.matchAll(/<picture\b[\s\S]*?<\/picture>/gi)];
    for (const [block] of pictures) total += heaviest(block);
    const rest = source.replace(/<picture\b[\s\S]*?<\/picture>/gi, "");
    for (const [tag] of rest.matchAll(/<img\b[^>]*>/gi)) total += heaviest(tag);
    return total;
  };
  ...
  for (const page of html) {
    const images = imageCost(page);
    const total = bytes(page) + shared + images;
    const budget = rel(page) === "dist/index.html" ? 500_000 : 1_000_000;
    const ok = total <= budget;
    if (!ok) {
      fail("V4", `${rel(page)} is ${total} bytes against a ${budget} budget`);
      if (images) fail("V4", `  ${images} of that is photography. Fewer images on the page, not a bigger budget.`);
    }
    console.log(
      `      ${rel(page).padEnd(24)} ${String(total).padStart(7)} bytes / ${budget}` +
        `${images ? ` (${images} images)` : ""}  ${ok ? "within budget" : "OVER"}`
    );
  }
}
...
/* ---------- V14 — images: reserved space, real files, and a ceiling ----------
 *
 * Three failures that share one property: none of them is visible from here. The
 * build is happy, the gates are green, the page renders, and the cost lands on a
 * buyer in Nairobi on a metered bundle who never tells anyone.
 *
 * WIDTH AND HEIGHT ON EVERY <img>. Without them the browser reserves no space
 * until the bytes arrive, and everything under the image jumps when they do. On
 * a product page that is the price, the docket and the add-to-order button
 * moving under a thumb mid-tap. On a fast connection you will never see it; on
 * the connections this site is built for it is most of the first second. The
 * attributes cost nothing and are already in products.json, read off the encoded
 * file — a component cannot guess them, which is why they live in the data.
 *
 * EVERY IMAGE URL RESOLVES. Gallery.astro derives five of each photograph's six
 * URLs from the sixth by pattern, and its RENDITIONS list is a copy of WIDTHS in
 * scripts/images.mjs. Duplicated constants drift; so this resolves what the page
 * actually emits against what is actually in dist/, which catches the drift, a
 * typo, and a rendition that was pruned while a page still pointed at it. It is
 * V12's argument applied to images, and the stakes are higher: a dead link is
 * discovered by clicking, a dead <img> is a broken picture on the page whose job
 * is to prove there is a real product behind it.
 *
 * 200KB PER FILE. The homepage budget is 500KB and any page 1MB, and those are
 * pass/fail in CLAUDE.md rather than aspirations. One careless export — a
 * 4000px master dropped into public/img/, a quality setting nudged up — spends
 * the whole homepage on one photograph. scripts/images.mjs already refuses to
 * write a file over this, so reaching this check means a file arrived some other
 * way, which is exactly the case a script cannot guard.
 *
 * The ceiling is on public/ rather than dist/ on purpose: that is where the file
 * enters the repo and where it will be committed, and catching it after it is
 * already in git history is catching it too late.
 */

{
  const IMAGE = /\.(avif|webp|png|jpe?g|gif|svg)$/i;
  const IMG_DIR = join(ROOT, "public", "img");
  const CEILING = 200 * 1024;

  /* ---- the ceiling, on the source of truth ---- */

  let heaviest = 0;
  let counted = 0;

  if (existsSync(IMG_DIR)) {
    for (const file of walkAll(IMG_DIR)) {
      const size = statSync(file).size;
      counted++;
      heaviest = Math.max(heaviest, size);
      if (size > CEILING) {
        fail("V14", `${rel(file)} is ${size} bytes against a ${CEILING} ceiling`);
        fail("V14", "  One photograph must not spend the homepage's whole 500KB budget.");
        fail("V14", "  Re-run scripts/images.mjs rather than hand-placing a file here.");
      }
    }
  }

  console.log(
    `  V14 image ceiling     ${counted} file(s) in public/img/` +
      `${counted ? `, heaviest ${heaviest} bytes / ${CEILING}` : ""}`
  );

  /* ---- what the built pages actually ask for ---- */

  if (existsSync(DIST)) {
    const built = new Set(walkAll(DIST).map((f) => "/" + rel(f).replace(/^dist\//, "")));
    const pages = walkAll(DIST).filter((f) => f.endsWith(".html"));
    ...
    for (const page of pages) {
      ...
      for (const [tag] of html.matchAll(/<img\b[^>]*>/gi)) {
        ...
        for (const attr of ["width", "height"]) {
          if (!new RegExp(`\\s${attr}=`).test(tag)) {
            fail("V14", `${rel(page)}  <img> with no ${attr} — nothing reserves its space`);
            fail("V14", "  Every photograph's real dimensions are in products.json. Use them.");
          }
        }
        if (!/\salt=/.test(tag)) {
          fail("V14", `${rel(page)}  <img> with no alt — say what the photograph shows`);
        }
      }

      /* src on <img>, and every candidate in every srcset on <img> and <source>.
         A srcset the browser never picks on this device is still one it will
         pick on another. */
      const candidates = [];
      for (const [, src] of html.matchAll(/<img\b[^>]*\ssrc=["']([^"']+)["']/gi)) candidates.push(src);
      ...
      for (const url of candidates) {
        ...
        urls++;
        if (!built.has(url)) missing.add(`${rel(page)} -> ${url}`);
      }
    }

    for (const m of missing) fail("V14", `image not built  ${m}`);
    if (missing.size) {
      fail("V14", "  A broken photograph on a product page is worse than a missing one:");
      fail("V14", "  it says the shop is neglected rather than that the shoot has not happened.");
      fail("V14", "  Check RENDITIONS in Gallery.astro against WIDTHS in scripts/images.mjs.");
    }

    console.log(
      `  V14 rendered images   ${imgs} <img> across ${pages.length} page(s), ` +
        `${urls} URL(s) resolved${missing.size ? `, ${missing.size} missing` : ""}` +
        `${imgs === 0 ? " — every product still renders light" : ""}`
    );
  }

  /* A rendition in public/img/ that no page references is weight in the repo and
     in the deploy, paying for nothing. The prune in scripts/images.mjs handles
     the ordinary case — a re-shoot supersedes the old hash — so this is the
     case the prune cannot see: a product whose photos array was emptied, or a
     file placed here by hand. A note, not a failure: the file is harmless where
     a broken reference is not, and there is a legitimate window where a
     photograph is committed just before the data that points at it. */
  if (existsSync(IMG_DIR) && existsSync(DIST)) {
    const referenced = new Set();
    for (const page of walkAll(DIST).filter((f) => f.endsWith(".html"))) {
      const html = readFileSync(page, "utf8");
      for (const [, src] of html.matchAll(/<img\b[^>]*\ssrc=["']([^"']+)["']/gi)) referenced.add(src);
      for (const [, set] of html.matchAll(/\ssrcset=["']([^"']+)["']/gi)) {
        for (const part of set.split(",")) referenced.add(part.trim().split(/\s+/)[0]);
      }
    }
    const orphans = walkAll(IMG_DIR)
      .map((f) => "/" + rel(f).replace(/^public\//, ""))
      .filter((u) => IMAGE.test(u) && !referenced.has(u));
    if (orphans.length) {
      notes.push(
        `${orphans.length} file(s) in public/img/ are referenced by no built page:\n` +
          orphans.map((o) => `          ${o}`).join("\n") +
          "\n        Weight in the repo and in the deploy, paying for nothing. Delete them, or\n" +
          "        re-run scripts/images.mjs for the product they belong to."
      );
    }
  }
}
